...
export default function UpcomingMovie() {
  ...
  const [totalPages, setTotalPages] = useState();
  const [expandedMovie, setExpandedMovie] = useState("");
  const [getbookmark, setgettingbookmark] = useState();
  ...
  useEffect(() => {
    getbookmarkbyuser().then((res) => {
      if (isuserLoggedIn) {
        setgettingbookmark(res.data.bookmarks);
      }
    });
  }, [isuserLoggedIn]);
  useEffect(() => {
